...
//   app.get('/api/sports', authenticateUser, async (req: Request, res: Response) => {
//     try {
...
//       const sports = await db.selectDistinct({ sport: coaches.sport }).from(coaches);
...
//   app.get('/api/positions/:sport', authenticateUser, async (req: Request, res: Response) => {
//     try {
...
//         "Mens Basketball": ["Point Guard", "Shooting Guard", "Small Forward", "Power Forward", "Center"],
//         "Mens Golf": ["Golfer"],
//         "Mens Ice Hockey": ["Goaltender", "Defenseman", "Center", "Left Wing", "Right Wing"],
//         "Mens Lacrosse": ["Attackman", "Midfielder", "Defenseman", "Goalie", "Faceoff Specialist", "Long Stick Midfielder"],
...
//         "Womens Basketball": ["Point Guard", "Shooting Guard", "Small Forward", "Power Forward", "Center"],
//         "Womens Golf": ["Golfer"],
//         "Womens Ice Hockey": ["Goaltender", "Defenseman", "Center", "Left Wing", "Right Wing"],
...
//         "Womens Volleyball": ["Outside Hitter", "Opposite Hitter", "Setter", "Middle Blocker", "Libero", "Defensive Specialist"],
...
//         .filter(pos => pos && !pos.toLowerCase().includes('coach') && !pos.toLowerCase().includes('director'));
...
//       console.log(`User profile accessed: ID=${user.id}, Email=${user.email}`);
...
//           authorization: req.headers.authorization ? 'present' : 'not present',
//           contentType: req.headers['content-type'],
//           firebaseUid: req.headers['x-firebase-uid'] ? 'present' : 'not present'
//         },
...
//         console.log(`Attempting to verify auth token for profile update, length: ${token?.length || 0}`);
...
//           }
//         } catch (tokenError) {
//           console.error('Token verification error in profile update:', tokenError);
//         }
//       }
...
//         console.log(`Attempting fallback authentication with Firebase UID header: ${firebaseUid}`);

//         if (firebaseUid) {
//           const user = await storage.getUserByFirebaseUid(firebaseUid);
//           if (user) {
//             userId = user.id;
//             console.log(`Found user ID ${userId} from X-Firebase-UID header: ${firebaseUid}`);
//           }
//         }
//       }
...
//       const useDevMode = isDevelopment && (req.query.dev === 'true');
...
//       if (!userId) {
//         console.log("No user ID found - authentication required for profile update");
...
//           firebase_uid_header: req.headers['x-firebase-uid'] ? 'present' : 'not present',
...
//       console.log(`User data retrieved for ID=${userId}, Email=${user.email || "none"}`);
//       res.json(userWithoutPassword);
//     } catch (error) {
...
//       else if (req.headers.authorization) {
//         const token = req.headers.authorization.split(' ')[1];
//         console.log(`Attempting to verify auth token for user update, length: ${token?.length || 0}`);
...
//       // Require proper authentication
//       if (!userId) {
//         console.log("No user ID found - authentication required for user update");
...
//       console.log("User update data (cleaned):", JSON.stringify(updateData, null, 2));
...
//   app.post("/api/coaches/update-conferences", authenticateUser, async (req, res) => {
//     try {
//       console.log("Starting coach conference update from CSV...");
//       // Use the actual path to the CSV file
//       const csvFilePath = './attached_assets/recruitref_dashboard_full_multithreaded.csv';
...
//       console.log(`Received request for coaches with pagination: page=${page}, limit=${limit}`);
...
//             console.log(`Automatically filtering by user's sport: ${userSport}`);
//           }
//         } catch (error) {
...
//       const existingSaved = await storage.getSavedCoachByUserAndCoach(userId, coachId);
...
//       const divisions = [...new Set(result.coaches
//         .map(coach => coach.division)
//         .filter(Boolean))]
//         .sort();
...
//       const result = await storage.getCoaches(filters);
//       const conferences = [...new Set(result.coaches
//         .map(coach => coach.conference)
//         .filter(Boolean))]
//         .sort();
...
//       const result = await storage.getCoaches(filters);
//       const regions = [...new Set(result.coaches
//         .map(coach => coach.region)
//         .filter(Boolean))]
//         .sort();
...
//       const result = await storage.getCoaches(filters);
//       const states = [...new Set(result.coaches
//         .map(coach => coach.state)
//         .filter(Boolean))]
//         .sort();
...
//       const savedCoach = await storage.getSavedCoachByUserAndCoach(userId, coachId);
...
//       if (error) {
//         return res.status(400).json({ message: "Invalid coach data", errors: error });
//       }
...
//       if (error) {
//         return res.status(400).json({ message: "Invalid email template data", errors: error });
//       }
...
//       const { userProfile, coachInfo, tone, coachId, teamPerformanceData } = req.body;
...
//           .map(([key, value]) => `${key}: ${value}`)
//           .join('\n');
//       } else if (typeof teamPerformanceData === 'string' && teamPerformanceData.trim() !== '') {
...
//   app.post("/api/emails", authenticateUser, async (req, res) => {
//     try {
//       // Require authentication
//       if (!req.user || !req.user.id) {
//         return res.status(401).json({ 
//           message: "Authentication required to create emails",
//           code: "AUTH_REQUIRED"
//         });
//       }
...
//       // Get the coach IDs from the request
//       const coachIds = Array.isArray(req.body.coachIds) ? req.body.coachIds : [req.body.coachIds];

//       if (!coachIds || coachIds.length === 0) {
//         return res.status(400).json({ message: "At least one coach ID is required" });
//       }

//       // For multiple coaches, we'll create an email for each coach
//       const emails = [];
...
//           body: req.body.body,
//           sentAt: new Date(),  // Use Date object directly, not string
//           status: req.body.isDraft ? "draft" : "sent",
...
//           // Create activity for non-draft emails
//           if (emailData.status === "sent") {
//             try {
//               const coach = await storage.getCoach(Number(coachId));
//               if (coach) {
//                 await storage.createActivity({
//                   userId,
//                   coachId: Number(coachId),
...
//           }
//         } catch (emailError) {
//           console.error(`Error creating email for coach ${coachId}:`, emailError);
//           // Continue with other coaches even if one fails
//         }
//       }

//       // If we didn't create any emails, return error
//       if (emails.length === 0) {
//         return res.status(500).json({ message: "Failed to create any emails" });
//       }

//       // Return the first email for a single email operation, or all emails for batch
//       res.status(201).json(emails.length === 1 ? emails[0] : emails);
//     } catch (error) {
...
//       res.status(201).json({ sent: sentEmails.length, emails: sentEmails, activities });
//     } catch (error) {
...
//       if (effectiveUserId === undefined && process.env.NODE_ENV === 'development') {
...
//         return res.status(401).json({ message: "Unauthorized - must be logged in to view activities" });
//       }
...
//       console.log("Fetching activities for userId:", effectiveUserId, "with limit:", limit);

//       // Ensure the userId is a number
//       const numericUserId = Number(effectiveUserId);
//       if (isNaN(numericUserId)) {
//         console.error(`Invalid userId for activities: ${effectiveUserId} (Not a number)`);
...
//       console.log(`Retrieved ${activities.length} activities for user ${numericUserId}`);
...
//         email.status !== 'draft' && 
//         email.status !== null && 
//         (email.status === 'sent' || email.status === 'replied')
//       );
...
//       const responses = emails.filter(email => email.status === 'replied').length;
...
//   app.get('/api/emails', authenticateUser, async (req: Request, res: Response) => {
//     try {
...
//       const emailsWithCoaches = await Promise.all(emails.map(async (email) => {
...
//   app.get('/api/emails/:id', authenticateUser, async (req: Request, res: Response) => {
//     try {
//       const userId = process.env.NODE_ENV === 'development' ? 1 : req.user?.id;
...
//       if (error) {
//         return res.status(400).json({ message: "Invalid email data", errors: error });
//       }
...
//       const savedCoach = await storage.getSavedCoachByUserAndCoach(userId, coach.id);
//       if (savedCoach && savedCoach.status === "Not Contacted") {
//         await storage.updateSavedCoach(savedCoach.id, { status: "Contacted" });
//       }
...
//   app.post('/api/emails/import-responses', authenticateUser, async (req: Request, res: Response) => {
//     try {
//       const userId = process.env.NODE_ENV === 'development' ? 1 : req.user?.id;
...
//       if (error) {
//         return res.status(400).json({ message: "Invalid response data", errors: error });
//       }
...
//       res.status(500).json({ message: "Error importing email responses", error: String(error) });
//     }
//   });

//   // Test webhook endpoint - useful for local testing of the webhook functionality
//   app.get('/api/webhook-test', authenticateUser, async (req: Request, res: Response) => {
//     try {
...
//         coachName: coach ? `${coach.firstName} ${coach.lastName}` : 'Not found'
//       });

//       if (!user || !coach) {
//         console.log("No users or coaches found for testing");
//         return res.status(404).json({ message: 'No users or coaches found for testing' });
//       }
