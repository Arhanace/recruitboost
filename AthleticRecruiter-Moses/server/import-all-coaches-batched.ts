...
async function importAllCoachesBatched() {
  try {
    ...
    console.log(`Starting batched coach import from ${filePath}...`);
    console.log(`Will import approximately ${totalRecords} coaches in batches of ${batchSize}`);
    
    // Only clear coaches table in the first batch
    ...
    while (importedSoFar < totalRecords) {
      ...
      // Only clear data for the first batch
      clearExisting = false;
