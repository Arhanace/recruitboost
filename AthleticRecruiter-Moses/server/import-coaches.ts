    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      const coachRecords = batch.map(record => {
        ...
        return {
          ...
          sport: record['Sport'] || '',
          position: record['Position'] || record['Coach Role'] || '',
      ...
      // 11 parameters for each coach record: first_name, last_name, email, phone, school, sport, position, division, conference, state, region
...
if (import.meta.url === `file://${process.argv[1]}`) {
  (async () => {
    try {
      ...
      const result = await importCoaches(filePath, limit);
