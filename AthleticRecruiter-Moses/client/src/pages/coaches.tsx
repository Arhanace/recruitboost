...
export default function Coaches() {
  ...
  // Set the sportFilter to the user's sport when profile is loaded
  useEffect(() => {
    if (userProfile?.sport) {
      setSportFilter(userProfile.sport);
      // Also set the saved sport filter to match the user's sport
      setSavedSportFilter(userProfile.sport);
    }
  }, [userProfile]);
  ...
  // Helper function to format coach names into Title Case
  ...
  // Get unique sports from coaches
  const sportOptions = coaches 
    ? Array.from(new Set(coaches.map((coach: Coach) => coach.sport)))
    : [];
  
  // Fetch division options from API endpoint
  const { data: divisionOptions = [] } = useQuery({
    queryKey: ["/api/coaches/divisions"],
    queryFn: async () => {
      const response = await fetch("/api/coaches/divisions");
      if (!response.ok) throw new Error("Failed to fetch divisions");
      return response.json();
    }
  });
    
  // Fetch conference options from API endpoint
  const { data: conferenceOptions = [] } = useQuery({
    queryKey: ["/api/coaches/conferences"],
    queryFn: async () => {
      const response = await fetch("/api/coaches/conferences");
      if (!response.ok) throw new Error("Failed to fetch conferences");
      return response.json();
    }
  });
    
  // Get unique roles from coaches
  const roleOptions = coaches
    ? Array.from(new Set(coaches.map((coach: Coach) => coach.position).filter(Boolean)))
    : [];
            ...
            <div>
              ...
              {userProfile?.role === 'admin' ? (
                ...
                <div className="flex items-center border rounded-md px-3 py-2 bg-gray-50">
                  ...
                  <span className="text-sm">{savedSportFilter || userProfile?.sport || "Loading..."}</span>
              ...
              <DialogContent className="sm:max-w-md">
                  ...
                  <div className="grid gap-2">
                    ...
                    <Select value={conferenceFilter} onValueChange={setConferenceFilter}>
                      ...
                      <SelectContent className="max-h-60 overflow-y-auto">
                        ...
                        {conferenceOptions.map((conference: string) => (
                ...
                <DialogFooter>
                  <Button type="submit" onClick={() => {
                    // Force a complete refetch of the data with the new filters
